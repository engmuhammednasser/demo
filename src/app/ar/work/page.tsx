        ...
        {projects.filter(p => !p.hidden && !p.type.includes("laravel") && !p.type.includes("php")).length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.filter(p => !p.hidden && !p.type.includes("laravel") && !p.type.includes("php")).map((project) => (
              <div 
                key={project.slug} 
                className="group flex flex-col bg-[#111827] rounded-2xl overflow-hidden border border-white/5 hover:border-[#38BDF8]/30 transition-all hover:shadow-[0_10px_30px_rgba(56,189,248,0.05)] hover:-translate-y-1 relative"
              >
                <Link href={`/${locale}/work/${project.slug}`} className="absolute inset-0 z-0" aria-label={`View ${project.title}`}></Link>
                <div className="relative aspect-[4/3] w-full bg-[#0B1020] overflow-hidden pointer-events-none">
